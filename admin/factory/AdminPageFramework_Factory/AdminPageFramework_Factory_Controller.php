 ...
 */
abstract class mhelpdeskAdminPageFramework_Factory_Controller extends mhelpdeskAdminPageFramework_Factory_View {
    ...
    public function setFieldErrors($aErrors) {
        $GLOBALS['amhelpdeskAdminPageFramework']['aFieldErrors'] = $this->oUtil->getElement($GLOBALS, array('amhelpdeskAdminPageFramework', 'aFieldErrors'), array());
        if (empty($GLOBALS['amhelpdeskAdminPageFramework']['aFieldErrors'])) {
            add_action('shutdown', array($this, '_replyToSaveFieldErrors'));
        }
        $_sID = md5($this->oProp->sClassName);
        $GLOBALS['amhelpdeskAdminPageFramework']['aFieldErrors'][$_sID] = isset($GLOBALS['amhelpdeskAdminPageFramework']['aFieldErrors'][$_sID]) ? $this->oUtil->uniteArrays($GLOBALS['amhelpdeskAdminPageFramework']['aFieldErrors'][$_sID], $aErrors) : $aErrors;
    }
    public function hasFieldError() {
        return isset($GLOBALS['amhelpdeskAdminPageFramework']['aFieldErrors'][md5($this->oProp->sClassName) ]);
    }
    public function setSettingNotice($sMessage, $sType = 'error', $asAttributes = array(), $bOverride = true) {
        $GLOBALS['amhelpdeskAdminPageFramework']['aNotices'] = $this->oUtil->getElement($GLOBALS, array('amhelpdeskAdminPageFramework', 'aNotices'), array());
        if (empty($GLOBALS['amhelpdeskAdminPageFramework']['aNotices'])) {
            add_action('shutdown', array($this, '_replyToSaveNotices'));
        }
        $_sID = md5(trim($sMessage));
        if ($bOverride || !isset($GLOBALS['amhelpdeskAdminPageFramework']['aNotices'][$_sID])) {
            ...
            $GLOBALS['amhelpdeskAdminPageFramework']['aNotices'][$_sID] = array('sMessage' => $sMessage, 'aAttributes' => $_aAttributes + array('class' => $sType, 'id' => $this->oProp->sClassName . '_' . $_sID,),);
        }
    }
    public function hasSettingNotice($sType = '') {
        $_aNotices = $this->oUtil->getElementAsArray($GLOBALS, array('amhelpdeskAdminPageFramework', 'aNotices'), array());
        if (!$sType) {
